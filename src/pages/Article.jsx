    ...
    <>
      ...
      <article className="mx-auto max-w-4xl px-4 py-8 sm:px-6 sm:py-12">
        ...
        <div className="mt-4 flex flex-wrap items-center gap-2 border-y border-slate-200 py-3">
          ...
          <button type="button" onClick={toggleLike} aria-label="Like article" className={`grid h-12 w-12 place-items-center rounded-lg border text-lg ${liked ? "border-blue-300 bg-blue-50" : "border-slate-200 bg-white"}`}>👍</button>
          <button type="button" onClick={() => document.getElementById("article-comments")?.scrollIntoView({ behavior: "smooth" })} aria-label="Comment on article" className="grid h-12 w-12 place-items-center rounded-lg border border-slate-200 bg-white text-lg text-slate-700">▤</button>
          <a href={`https://wa.me/?text=${encodeURIComponent(`${displayTitle} ${window.location.href}`)}`} target="_blank" rel="noopener noreferrer" aria-label="Share on WhatsApp" className="grid h-12 w-12 place-items-center rounded-lg border border-slate-200 bg-white text-lg font-black text-green-600">●</a>
          <button type="button" onClick={shareArticle} aria-label="Share article" className="grid h-12 w-12 place-items-center rounded-lg border border-slate-200 bg-white text-lg font-bold text-slate-700">↗</button>
          <button type="button" onClick={toggleSave} aria-label="Bookmark article" className={`grid h-12 w-12 place-items-center rounded-lg border border-slate-200 bg-white text-lg ${bookmarked ? "text-red-600" : "text-slate-700"}`}>♡</button>
          <div className="relative"><button type="button" onClick={() => setShowMore((current) => !current)} aria-label="More article actions" className="grid h-12 w-12 place-items-center rounded-lg border border-slate-200 bg-white text-lg text-slate-700">⋮</button>{showMore && <div className="absolute right-0 top-14 z-10 w-44 rounded-xl border border-slate-200 bg-white p-2 text-sm shadow-xl"><button type="button" onClick={() => navigator.clipboard.writeText(window.location.href)} className="block w-full rounded-lg px-3 py-2 text-left hover:bg-slate-100">Copy link</button><Link to="/contact" className="block rounded-lg px-3 py-2 hover:bg-slate-100">Report / Contact</Link></div>}</div>
        </div>
