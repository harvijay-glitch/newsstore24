...
export const searchNews = async (query) => {
  try {
    const results = await News.find({
      ...
      .select("title image url publishedAt category")
      .limit(10)
      .lean();
...
export const getNewsByCategory = async (category, limit = 20, skip = 0) => {
  try {
...
export const getLatestNews = async (limit = 20) => {
  try {
    const news = await News.find({ publishStatus: "published" })
      .sort({ publishedAt: -1 })
      .limit(limit)
      .lean();
